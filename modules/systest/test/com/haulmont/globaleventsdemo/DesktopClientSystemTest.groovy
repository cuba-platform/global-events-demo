...
import com.haulmont.globaleventsdemo.jmx.CoreTester
import com.haulmont.globaleventsdemo.jmx.CoreTester
...
class DesktopClientSystemTest {
    ...
    void testBeanEvent() {
        ...
        assertEquals(1, rows.findAll { it.receiver == 'com.haulmont.globaleventsdemo.core.FooCoreBean' }.size())
        assertEquals(1, rows.findAll { it.receiver == 'com.haulmont.globaleventsdemo.web.FooWebBean' }.size())
        assertEquals(1, rows.findAll { it.receiver == 'com.haulmont.globaleventsdemo.portal.FooPortalBean' }.size())
        assertEquals(1, rows.findAll { it.receiver == 'com.haulmont.globaleventsdemo.desktop.FooDesktopBean' }.size())
        assertTrue(rows.every { it.event_class == 'com.haulmont.globaleventsdemo.BeanNotificationEvent' })
